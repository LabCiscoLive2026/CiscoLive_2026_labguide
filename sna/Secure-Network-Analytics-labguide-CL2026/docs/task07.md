# Task 7: Analyzing Top Flows — Who Accessed What, How, and For How Long

## Objective

In this task, you will use a Splunk query to analyze the top network flows across your environment. While Task 1 identified the busiest devices, this task answers the deeper questions about network activity:
...
1. If you are continuing from Task 1, click the **Search** bar to clear the previous query.
...
5. **How does this relate to the top utilized hosts from Task 1?** By combining the insights from this query with the Top Utilized Hosts query from Task 1, you can build a complete picture of not only which devices are most active, but also how and why they are active — providing actionable intelligence for data center asset management.

## Result

In this task, you analyzed the top network flows to understand who is accessing what, how they are accessing it, and how long the access spanned. This detailed flow-level visibility complements the host-level view from Task 1. [**REVISIT**]

## Lab Summary

In this lab, you used Splunk Cloud and Cisco Secure Network Analytics to gain comprehensive visibility into asset utilization across your network:

| **Task** | **What You Did**| **Key Insight** |
| --- | --- | --- |
| Task 1: Top Utilized Hosts | Identified the busiest devices by unique flow count and total bytes | Reveals which assets are consuming the most network resources |
| Task 2: Top Flows Analysis | Analyzed who accessed what, using which protocol/port, and for how long | Reveals the specific communication patterns driving device utilization |

Together, these two tasks demonstrate how NetFlow telemetry collected by Cisco Secure Network Analytics can be exported to Splunk Cloud for centralized analysis, providing a powerful and scalable approach to asset utilization monitoring across distributed data center environments.

**Key takeaways from this lab:**

- **Cisco SNA** provides deep network flow visibility by collecting and analyzing NetFlow data from all monitored devices
- **Splunk Cloud** enables flexible querying, visualization, and dashboarding of SNA telemetry for operational insights
- **Asset utilization monitoring** requires both host-level (which devices are busiest) and flow-level (how and why they are busy) analysis
- The combination of SNA and Splunk Cloud provides a data-driven foundation for capacity planning, security auditing, and operational troubleshooting
